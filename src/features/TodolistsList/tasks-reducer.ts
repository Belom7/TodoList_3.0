...
const slice = createSlice({
    ...
    reducers: {
        removeTask: (state, action: PayloadAction<{ taskId: string, todolistId: string }>) => {
            ...
            // if (index !== -1) tasksForTodoLists.splice(index, 1)
        ...
        updateTask: (state, action: PayloadAction<{ taskId: string, model: UpdateDomainTaskModelType, todolistId: string }>) => {
            ...
            if (index !== -1) {
                tasksForTodoLists[index] = {...tasksForTodoLists[index], ...action.payload.model}
            }
            // let task = tasksForTodoLists.find(task => task.id === action.payload.taskId)
            // if (task) task.title = action.payload.model.title ?? ''
